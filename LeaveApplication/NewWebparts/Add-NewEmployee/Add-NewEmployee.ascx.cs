...
namespace LeaveApplication.NewWebparts.Add_NewEmployee
{
    [ToolboxItemAttribute(false)]
    public partial class Add_NewEmployee : WebPart
    {
                ...
                try
                {
                    using (var site = new SPSite(SPContext.Current.Site.Url))
                    {
                        using (var web = site.OpenWeb())
                        {
                            ...
                            {
                                Query = @"<Where><Eq><FieldRef Name='Title' /><Value Type='Text'>" +empdupID + @"</Value></Eq></Where>"
                            };
