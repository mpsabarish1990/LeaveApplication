...
namespace LeaveApplication.AllEmployeeLeaveList
{
    ...
    public partial class AllEmployeeLeaveList : WebPart
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            LoadLeaveDetails(hdnCurrentYear.Value);
        ...
        private void LoadLeaveDetails(string yearvalue)
        {
            try
            {
            ...
            try
            {
                using (var site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (var web = site.OpenWeb())
                    {
                        ...
                        var currentyearlist = web.Lists.TryGetList(Utilities.CurrentYear);

                        var currentyearvalue = "";

                        SPListItemCollection currentyearcoll = currentyearlist.GetItems();

                        foreach (SPListItem currentyearitem in currentyearcoll)
                        {
                            currentyearvalue = currentyearitem["Title"].ToString();
                        }
                                ...
                                currentUserDetails = leaveList.GetItems(new SPQuery
                                {
                                    Query = @" <Where>
                                          <And>
                                             ...
                                             <Eq>
                                                <FieldRef Name='Year' />
                                                <Value Type='Text'>" + currentyearvalue + @"</Value>
                                             </Eq>
                                          </And>
                                       </Where>"
                                });
